...
`backfill_hetzner.py` 的 `report`／`reindex`／`data_versions_in_cache` 憑空多開一個空 DB，
並讓 `config._check_registry()` 的 `d.db in DB_FILES` 斷言失去意義。前例＝`score_io.open_score_stores`。
`spec/P1-B3-replay.md` §B3.2 的 DB 切分表也沒有 `features.db`（同 `market.db` 的處境，
`config.py` 對後者註明「§B3.2 未指派檔名」）。

§B3.2 的其餘三項照辦：四條 PRAGMA 逐字相同、複合 PK ＋ `WITHOUT ROWID`＋ date 索引、
以日為外層迴圈逐日寫入（不整批累積在記憶體）。
...
class FeatureStore:
    ...
    def write_day(self, day: ScanDay, data_version: str, *, rank_pool_size: int,
        ...
        """寫一個交易日。**同一個 `(data_version, 日期)` 重寫會覆蓋**（`INSERT OR REPLACE`），
        所以中斷後重跑該日是安全的；但**不會**刪掉該日在舊參數下寫的列——參數不同時
        `set_params()` 早就擋下來了。
