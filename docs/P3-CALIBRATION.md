...
`read_day`＋`ingest` 約 1,240 日（2020-01 起到 2024-12-31）加上 ingest 期間常駐的視窗與基本面橋（整段重播的
RSS 峰值 551 MiB 可當上界參考，但那含計分；本腳本不計分全市場）；逐日 ingest 的單日成本在生產規模沒有單獨量過，
**30 分鐘是目標、不是量測值**，以 Hetzner log（每 100 日印一次耗時與 RSS、報告記 `elapsed_s`／`rss_peak_mib`）為準。
另：Hetzner 報告在加入 ① 之後重跑，被標的組**可能多於現在的 3 組**（例如 2026-09-14 報告中個股四爻短線
reweighted 兩組的 q1＝中位數＝50.0000，**推測**其 50.0 佔比 > 20%，未實測）；診斷對所有爻與大盤列都能重算，
不限初爻。
