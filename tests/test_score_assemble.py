...
def test_added_key_in_dimensions_is_required(tmp_path, ps_twse, mkt):
    ...
    with pytest.raises(KeyError, match="batch_id"):
        assemble_row(ms, ps_twse.model_version(), DV, TV, dimensions_path=str(p))
    row = assemble_row(ms, ps_twse.model_version(), DV, TV, extra_keys={"batch_id": "b1"}, dimensions_path=str(p))
...
def test_market_row_shape(ps_twse, mkt):
    ms = score_market(mkt, ps_twse, "short")
    row = assemble_row(ms, ps_twse.model_version(), DV, TV)
...
def test_row_deterministic_and_json_serializable(ps_twse, stk):
    a = assemble_row(score_stock(synth_stock_inputs(), ps_twse, "mid"), ps_twse.model_version(), DV, TV, detail=True)
    b = assemble_row(score_stock(synth_stock_inputs(), ps_twse, "mid"), ps_twse.model_version(), DV, TV, detail=True)
    assert a == b
...
def test_model_version_binding_changes_row(ps_twse, mkt):
    ms = score_market(mkt, ps_twse, "short")
    r1 = assemble_row(ms, ps_twse.model_version(), DV, TV)
    ps2 = ps_twse.with_param(SCOPE_MARKET, "short", "2", "C", "new_high_low_ratio", d=1.0)
    r2 = assemble_row(score_market(mkt, ps2, "short"), ps2.model_version(), DV, TV)
    ...
    r3 = assemble_row(score_market(synth_market_inputs(), ps_twse, "short"), ps_twse.model_version(), DV, TV)
...
def test_unknown_line_gives_pending_hexagram_and_missing_base(ps_twse):
    ...
    row = assemble_row(score_market(inp, ps_twse, "mid"), ps_twse.model_version(), DV, TV)
...
def test_formal_lines_to_king_wen(ps_twse, mkt):
    ms = score_market(mkt, ps_twse, "short")
    row = assemble_row(ms, ps_twse.model_version(), DV, TV, formal_lines=[1, 1, 1, 0, 0, 0])
    assert row["king_wen"] == 11 and row["hexagram_name"] == "地天泰"
    with pytest.raises(ValueError):
        assemble_row(ms, ps_twse.model_version(), DV, TV, formal_lines=[1, 1, 1])
