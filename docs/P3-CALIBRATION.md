...
報告可信度的兩項核對已寫進附錄 B：不經過 `d` 的量（`F-高波動`、個股 `overheated`）前後側差異**恰為 0.0**
（原值、非四捨五入）——若分析工具把不相干的東西算進去，這兩項不會是零。

### 裁定 #62（使用者「照你建議」）

| 項目 | 裁定 |
|---|---|
| ⑥ 主卦／前瞻式之卦一致率 | **改以 ① 單爻差異判定**。一卦六爻、一爻不同整卦就不同，實測 ⑥ ≈ (1−①)^6（12 格差距見附錄 B）；10% 門檻套在卦上等於要求單爻差異約 1.74% 以下，與 ① 的門檻不相容 |
| ⑦ 候選名單與名次重疊 | **接受為 c／d 校準的預期效果**；`T0` 未納入（裁定 #59），`T0` 定案後另行重驗 |
| ⑧ 封頂／下限觸發率 | **接受**（超標一格：tpex/mid 的 `floor_applied`） |
...
所以附錄 B：①每個數字由 f-string 從 JSON 算出；②附錄裡的每一句**定性斷言**（「② 超標全部在
`scope=market`」「① 全部低於門檻」「⑧ 前後側分母相同」）都配一道守門，資料推翻它就 `AppendixError`
中止、rc=2，不會寫出一句錯的字；③`--check` 由 `tests/test_t717_appendix.py` 守「附錄＝重新產生」。
附錄 B 放在附錄 A 的 marker 之後、有自己的 marker；`rank_table.py` 的 splice 只換自己兩個 marker
之間，測試另守「重寫附錄 A 不吃掉附錄 B」。

突變實測（9 個，全數被預期的那支測試抓到）：拿掉 ②／⑧／半 marker 三道守門、`**6`→`**5`、
