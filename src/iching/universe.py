...
排除 FinMind 傘狀類別（`UMBRELLA_CATEGORIES`，例 `電子工業`）若尚有更細者，再依 (industry_category, type, stock_name)
字串排序取第一。`same_date_multi=True` 標出這種代號，report 列出檔數供人工複核。
...
def _pick(rows: list[dict]) -> dict:
    ...
    cands = finer or tied
    cands = sorted(cands, key=lambda r: (str(r.get("industry_category") or ""), str(r.get("type") or ""),
