...
const W = {   // window 字串（與 FACET 相同，展開只為讓預期句在測試裡肉眼可讀）
  ...
  s2: "短線 MA5／MA20、波段 MA10／MA20、中期 MA20／MA60，距離與斜率以 ATR14 為單位",
...
const cases = [
  ...
  ["14 每爻句 翻轉當日（三爻 40.0、sk 0、前一日為陽）→ 動爻句、無候選句", () => explainLine(2, E({ lf: "110111", st: "yynyyy", l: [70, 70, 40, 70, 70, 70] }), "stock", "yyyyyy"),
    `看什麼：對所屬指數的超額報酬（長短視窗）、對同業中位數的超額、超額加速度（${W.s3}）。目前分數 40.0，低於支持門檻，相對動能支持不足或偏弱。今日完成翻轉（動爻）。`],
  ["15 大盤每爻句 yin（初爻 30.0）", () => explainLine(0, E({ lf: "011111", st: "nyyyyy", l: [30, 70, 70, 70, 70, 70] }), "market", null),
    `看什麼：指數與短、長均線距離，均線斜率，指數在區間中的位置（${W.m1}）。目前分數 30.0，低於支持門檻，趨勢支持不足或偏弱。爻態穩定，分數未站到翻轉門檻另一側。`],
  ...
  ["17 lf／lp 皆 null → 退回 st；外卦含「-」列為未知", () => explainHex(E({ st: "yyy-nn" }), "stock", null),
    "內卦乾：三面向皆支持；外卦含未知爻，僅列已知：四 量價確認 未知、五 籌碼供需 未達門檻、上 外部支持 未達門檻。"],
  ...
  ["20 每爻句 個股初爻 yang（60.0）", () => explainLine(0, E({ lf: "111111", st: "yyyyyy", l: [60, 70, 70, 70, 70, 70] }), "stock", null),
    `看什麼：月營收年增、營收加速度；中期另看 EPS 年增、毛利率季變化、贏同業幅度（${W.s1}）。目前分數 60.0，落在支持區，營運基礎較支持上行。爻態穩定，分數未站到翻轉門檻另一側。`],
  ...
  ["21 sk=1 未翻轉 → 候選變化句（含「再 1 日」）", () => explainLine(3, E({ lf: "111111", st: "yyyyyy", l: [70, 70, 70, 44.0, 70, 70], sk: "0,0,0,1,0,0" }), "stock", "yyyyyy"),
    "看什麼：量比與當日漲跌、回撤的情境表，收盤位置，突破後是否守住（短線 5 日、波段 10 日、中期 20 日）。目前分數 44.0，低於支持門檻，量價確認支持不足或偏弱。候選變化：分數已連續 1 日站在翻轉門檻另一側，再 1 日仍站住即翻爻。"],
  ["22 sk=0 未翻轉 → 穩定句", () => explainLine(3, E({ lf: "111111", st: "yyyyyy", l: [70, 70, 70, 66.0, 70, 70] }), "stock", "yyyyyy"),
    "看什麼：量比與當日漲跌、回撤的情境表，收盤位置，突破後是否守住（短線 5 日、波段 10 日、中期 20 日）。目前分數 66.0，落在支持區，量價確認較支持上行。爻態穩定，分數未站到翻轉門檻另一側。"],
  ["23 翻轉當日 sk=0 → 動爻句、無候選句", () => explainLine(3, E({ lf: "111011", st: "yyynyy", l: [70, 70, 70, 40.0, 70, 70] }), "stock", "yyyyyy"),
    "看什麼：量比與當日漲跌、回撤的情境表，收盤位置，突破後是否守住（短線 5 日、波段 10 日、中期 20 日）。目前分數 40.0，低於支持門檻，量價確認支持不足或偏弱。今日完成翻轉（動爻）。"],
  ["24 sk=2 未翻轉（理論上不出現）→ 不炸、寫「再 1 日」", () => explainLine(3, E({ lf: "111111", st: "yyyyyy", l: [70, 70, 70, 44.0, 70, 70], sk: "0,0,0,2,0,0" }), "stock", null),
    "看什麼：量比與當日漲跌、回撤的情境表，收盤位置，突破後是否守住（短線 5 日、波段 10 日、中期 20 日）。目前分數 44.0，低於支持門檻，量價確認支持不足或偏弱。候選變化：分數已連續 2 日站在翻轉門檻另一側，再 1 日仍站住即翻爻。"],
];
...
const SPEC_TRI_STOCK = {   // spec/stock-iching-plan-v1.2.2.md:105-112 兩欄逐字
  "乾": ["三面向皆支持", "量價、籌碼、環境皆支持"],
  "兌": ["營運與趨勢支持，相對動能不足", "量價與籌碼支持，環境不足"],
  "離": ["營運與動能支持，趨勢未配合", "量價與環境支持，籌碼未配合"],
  "震": ["營運支持，價格面尚未配合", "量價支持，籌碼與環境不足"],
  "巽": ["價格面支持，營運尚未配合", "籌碼與環境支持，量價確認不足"],
  "坎": ["趨勢支持，其餘兩面向不足", "籌碼支持，量價與環境不足"],
  "艮": ["相對動能支持，營運與趨勢不足", "環境支持，個股交易條件不足"],
  "坤": ["三面向均未達支持門檻", "三面向均未達支持門檻"],
};
const FORBID = ["機率", "勝率", "看多", "看空", "買進", "賣出", "多頭", "空頭", "吉", "凶", "趨勢反轉", "亢龍有悔", "轉弱", "轉強"];
const structural = [
  ...
  ["TRI.market 8 卦；乾內卦／坤兩欄是不點名面向的通用句（兩側同字），其餘每格都與個股側不同字串", Object.keys(TRI.market).length === 8
    && TRI.market["坤"][0] === "三面向均未達支持門檻" && TRI.market["乾"][0] === "三面向皆支持"
    && Object.keys(TRI.stock).every(k => k === "坤" || (TRI.stock[k][1] !== TRI.market[k][1] && (k === "乾" || TRI.stock[k][0] !== TRI.market[k][0])))],
  ["FACET 個股／大盤各 6 爻、yang／yin＝面向名＋固定尾綴、名稱兩側互不相同", ["stock", "market"].every(k => FACET[k].length === 6
    && FACET[k].every(f => f.yang === f.name + "較支持上行" && f.yin === f.name + "支持不足或偏弱" && f.what && f.window))
  ...
  ["TERMS 遲滯條＝裁決文", TERMS[3][1] === "分數站到翻轉門檻另一側（陰→陽 ≥55、陽→陰 ≤45）後，還要連續兩個交易日站住才翻爻；站住 1 日的爻稱候選變化"],
];
