...
| 表 | 鍵 | 內容 |
|---|---|---|
| `scores` | 邏輯鍵＝`dimensions.json` 的 7 鍵（`market, horizon, stock_id, tpe_trading_date, model_version, data_version, text_version`），大盤列 `stock_id='__MARKET__'`；**物理 PK＝`(version_id, market, horizon, stock_id, date)`**（Q2 乙） | `assemble_row(detail=False)` 的全部欄位（46 欄（2026-09-21 §18 起））攤平：純量進真欄，`lines_provisional`／`lines_formal` 存 6 字元位元串（`"010010"`，下爻在前）、`flags` 進 JSON TEXT；另加 **`line_states` TEXT（6 字元 y/n/-，遲滯 state 本體）**、**`streaks` TEXT（6 個 int）** 與 **`in_rank_pool` INTEGER**（個股列；大盤列 NULL）（2026-09-13 實作後更正：原寫 `lines_*` 進 JSON） |
...
1. **13a-1** `replay_state.py`：`CrossDayState`（序列化＋載入）、`WindowCache`（含後復權接線）、`ingest(T)`。純函式層不 import sqlite3；I/O 在 `replay_io.py`。
   **已交付（2026-09-13）**：`src/iching/replay_state.py`（`DayBundle`／`CrossDayState`／`WindowCache`／`Ring`）＋ `src/iching/replay_io.py`（`ReplaySource.read_day(T)`：15 張表逐日點查詢，含官方 JSON 解析、FMTQIK／tradingIndex 月表快取、TX 近月基差、VIX 末筆、美股／匯率增量）＋ `features_io.FeatureStore(readonly=True)` 與 `day_breadth／day_industry／day_p_cs` 讀取器＋ `liquidity.AdvTracker.state()/from_state()`；測試 `tests/test_replay_state.py`（17 條，fixture `tests/synth_db.build_full`）。**實作時定下的三個語意**（都寫在 `replay_state.py` docstring）：①個股序列只在「有成交且所屬市場有指數列」推進（`score_io` 連停牌列也算，本檔與 `feed.day_records` 同尺）；②籌碼沿用 `score_io.aligned()`——視窗內完全無列→整欄 None、部分缺→法人補 0／餘額 NaN；③**`ad_line` 在視窗內從 0 起算**，否則每日班（只重建 320 日）與全量跑差一個常數、`(AD − MA)` 的浮點結果不逐位相等（測試 `test_window_rebuilt_from_last_n_days_is_bitwise_identical` 守）。`score_io.load_stock_ohlcv` 仍為原始價、只供量測對照，未動。
2. **13a-2** `replay_step.py`：`step(T)` 同日順序（§4）；`scores_io.py`：schema／writer。
   **已交付（2026-09-13）**：`src/iching/replay_step.py`（`step(T)` 逐字對應 §4：`adv.eligible()` 先取 → 兩市場 3 期間 `score_market` → 遲滯 → `basic_state` → 互填 `other_market_state` → `market_flags` → 讀完 T−5 才 push 大盤二爻 → 個股（`market_direction_score` 的 `Missing` 轉 None）→ push 個股二爻 → `adv.push_day` 後推）；`src/iching/scores_io.py`（`versions` 正規化表＋`scores` 46 欄（2026-09-21 §18 起）攤平＋`line_states`／`streaks`／`in_rank_pool`、`replay_day`、`replay_meta` 參數指紋拒混寫；逐日整日取代）；`universe.is_financial()`（裁定 #28 明列集合）；`WindowCache` 補 `shares_outstanding`（`raw_shareholding` 最近申報值）。測試 `tests/test_replay_step.py` 10 條：合成 DB 全量兩次逐位相同；**單步 parity**（T−1 的 `CrossDayState` JSON ＋只用最後 30 天重建的 `WindowCache` → `step(T)` 的 21 列與全量跑逐位相同、推進後狀態 JSON 逐字相同）；版本綁定（改 `direction_unknown_policy` 同日列不同、`versions` 表兩版並存、參數指紋拒混寫）；`P_cs` 原生值直達 `lines["3"].meta`；`in_rank_pool` PIT（第 60 日全 0、第 61 日起達門檻者 1）；大盤 T−5 二爻／個股 9 筆歷史與落地列一致。**尚未做**：`scripts/replay_scores.py` 驅動（13a-3）、Hetzner 實跑。
3. **13a-3** `scripts/replay_scores.py`：CLI（同構 `scan_features.py`：`--from/--to/--limit-days/--rebuild/--resume`，暖機拒跑）＋ `check_scores.py` 健檢。
   **已交付（2026-09-13）**：`scripts/replay_scores.py`（全量／`--limit-days`／`--rebuild`／`--resume`（讀 `<out>.state.json` 快照續跑）／`--from X --state s.json`；**沒有「從中間冷起跑」**：`--from` 一律要求前一交易日快照，否則拒跑；視窗重建起點由 `ReplaySource.rebuild_start()` 從資料倒推（每檔最近 window 個**有成交**日、各序列最近 window 個日期，取最早）——首版用「往前 window 個日曆交易日」，測試立刻抓到停牌過的 1102 在續跑第一日分數不同；每 `--progress-every` 日印 RSS）＋ `scripts/check_scores.py`（唯讀健檢：版本表、列數、每 400 日診斷取樣、step 耗時 p50/p90/max、各爻未知率、`lines_formal` 非 NULL 比例、末日大盤列／卦分布／coverage／排名池）。`ReplaySource.__init__` 新增 **date 索引守門**（§2「缺即拒跑」，回補後忘了 `reindex` 會退化成全表掃描）。測試 `tests/test_replay_scores.py` 6 條（全量落地＋快照；`--limit-days`／`--rebuild`；**`--resume` 續跑與一次跑完逐位相同、快照逐字相同**；`--from` 正確用法與三種拒跑；壞路徑 rc=2；缺索引拒跑）。另依 13a-1 驗收建議修：`DayBundle.stocks` 的 `volume`／`Trading_Volume` 二擇一、`tpe_dates` 守門測試、刪恆真式斷言、文件測試數 17。**13a-3 驗收（2026-09-13，綁 `09731bb`，8 條全 ✅、4 個突變全被抓）後修**：①`rebuild_start()` 個股候選改「有成交 ∩ 所屬市場有指數列」且限池內（原 docstring 說不取交集是安全方向——**說反了**，驗收實測缺一天指數列會少一列）；②`--window` 進快照 `meta`（連同 `params_sha`），`--from`／`--resume` 比對不符拒跑（原本 `--from` 進新 DB 配不同 window 會無聲產出不可重現的列）；③輸入快照（`--state`）不再被覆寫，輸出一律 `<out>.state.json`；④`save_state` 加 fsync。**尚未做**：Hetzner 實跑（需先 `reindex`）。
