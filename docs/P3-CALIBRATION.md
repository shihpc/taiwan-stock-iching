...
## 22. `:717` 重驗結果：裁定 #62 與附錄 B（2026-09-23）
...
| ② 遲滯翻爻次數、⑤ 內外卦方向判定差異率 | **未請使用者確認**——附錄 B 標「待使用者確認（凍結前必須補齊）」，不替使用者認定 |

確認後改 `scripts/t717_appendix.py` 的 `CONFIRMED` 再重跑，附錄文字自動跟著變。
...
守門共八道：② 超標全在 `scope=market`／② 大盤翻爻全部變多／不經 `d` 的兩項差為零／⑤ 整段高於 ①／
① 全部 ≤ 10%／⑥ 主卦與前瞻式之卦 24 個值與 (1−①)^6 的差都 ≤ `HEX_GAP_MAX`／⑧ `floor_applied` 前後分母相同／
⑧ 超標格後側較高。`HEX_GAP_MAX`（3 個百分點）是**附錄自訂的說明門檻，不是規格常數**。

**首版（`fdfe33e`）把這句寫成「每一句定性斷言都配一道守門」，驗收實查不成立**：當時只有三道，
「前後差異為零」「⑤ 高於 ①」「大盤翻爻變多」「後側較高」「⑥ 是 ① 帶出來的」五句都沒有守門，
另有一句無依據的解讀（「⑦ 是校準影響最大的一項」——⑥ 的 1−一致率最大值其實高於 ⑦ 的 1−Jaccard）
已刪除；⑥ 的說明原本只比主卦，前瞻式之卦的差距（範圍更寬）沒列出、結論卻涵蓋全部 24 處，現已並列。
同批把「待確認」測試從「數次數」改成「逐節綁定」（驗收 S1：⑤⑦ 確認狀態對調後，只數次數會全綠）。
附錄 B 放在附錄 A 的 marker 之後、有自己的 marker；`rank_table.py` 的 splice 只換自己兩個 marker
之間，測試另守「重寫附錄 A 不吃掉附錄 B」。

突變實測（首版 9 個＋補守門後 7 個，全數被預期的那支測試抓到）：拿掉 ②／⑧／半 marker 三道守門、`**6`→`**5`、
① 守門 `>`→`>=`（邊界測試抓）、⑤ 改成已確認、`--check` 失效、`rank_table` splice 吃到檔尾、
列數加總少加大盤列；補守門後逐一拿掉四道新守門（零差／⑤ 高於 ①／② 變多／⑧ 較高）、⑥ 只守主卦、⑥ 門檻 `<=`→`<`、⑤⑦ 確認狀態對調並重新生成。
