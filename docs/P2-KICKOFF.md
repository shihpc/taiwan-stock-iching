...
| 31 | **批一第 10／11 項落地時浮出的五個口徑選擇（2026-09-12）——⚠ 尚未裁定** | 全部**已先照下方括號內的值實作**（集中在 `src/iching/scan.py` 模組 docstring，標 SPEC-NOTE，改一處即可換掉），列此以免遺失；使用者裁定後再回填。**①漲跌口徑**（暫：後復權價）——官方「上漲家數」用原始價，差異只落在除權息日，影響 `advance_ratio`／`ad_line`／`up_amount_ratio`；混用會在除權息日出現「被判下跌但站上 MA」的自相矛盾。**差異幅度待第 12 項在 Hetzner 量測**。**②家數比分母**（暫：`N_t`，照 `P1-B1-market.md:157` 字面）——歷史不足 n 天的新股算在分母卻不可能在分子，有向下偏誤；`ma_eligible`／`hl_eligible` 診斷欄就是為量它而存在，**幅度待量測**。**③`up_amount_ratio` 分母**（暫：漲跌可判定子集，與分子同名單）——規格只寫「總成交金額」未指明母體；另存 `amount_total` 供改口徑。**④`P_cs` 平手規則**（暫：`"mid"`，同 `transform.P_hist`）——`P_cs` 經過熱旗標影響三爻分數，嚴格說該加 `Rules.p_cs_tie` 並進 `model_version` 指紋；**加欄位會動指紋，本批刻意未動**。**⑤產業中位數母體**（暫：廣度母體＝普通股全體，非排名池）——它是描述性統計，不該隨流動性門檻滾動而跳動。 |
...
> **第 31 列是唯一「尚未裁定」的一列**，其餘皆為已裁定紀錄。它列的五項已有暫行實作、不擋後續開發，
> 但**凍結預先登錄書之前必須裁定**——其中 ④ 會改動 `model_version` 指紋，①②③ 會改動分數。
