...
const cases = [
  ...
  ["21 sk=1 未翻轉 → 候選變化句（含「再 1 日」）", () => explainLine(3, E({ lf: "111111", st: "yyyyyy", l: [70, 70, 70, 44.0, 70, 70], sk: "0,0,0,1,0,0" }), "stock", "yyyyyy"),
    "看什麼：成交量／均量與當日漲跌、回檔的情境表，收盤位置，突破後是否站穩（短線 5 日、波段 10 日、中期 20 日）。目前分數 44.0，偏空區，量價確認支撐不足或偏弱。候選變化：分數已連續 1 日站在翻轉門檻另一側，再 1 日仍站住即翻爻。"],
  ["22 sk=0 未翻轉 → 穩定句", () => explainLine(3, E({ lf: "111111", st: "yyyyyy", l: [70, 70, 70, 66.0, 70, 70] }), "stock", "yyyyyy"),
    "看什麼：成交量／均量與當日漲跌、回檔的情境表，收盤位置，突破後是否站穩（短線 5 日、波段 10 日、中期 20 日）。目前分數 66.0，偏多區，量價確認較有利上漲。爻態穩定，分數未站到翻轉門檻另一側。"],
  ["23 翻轉當日 sk=0 → 動爻句、無候選句", () => explainLine(3, E({ lf: "111011", st: "yyynyy", l: [70, 70, 70, 40.0, 70, 70] }), "stock", "yyyyyy"),
    "看什麼：成交量／均量與當日漲跌、回檔的情境表，收盤位置，突破後是否站穩（短線 5 日、波段 10 日、中期 20 日）。目前分數 40.0，偏空區，量價確認支撐不足或偏弱。今日完成翻轉（動爻）。"],
  ["24 sk=2 未翻轉（理論上不出現）→ 不炸、寫「再 1 日」", () => explainLine(3, E({ lf: "111111", st: "yyyyyy", l: [70, 70, 70, 44.0, 70, 70], sk: "0,0,0,2,0,0" }), "stock", null),
    "看什麼：成交量／均量與當日漲跌、回檔的情境表，收盤位置，突破後是否站穩（短線 5 日、波段 10 日、中期 20 日）。目前分數 44.0，偏空區，量價確認支撐不足或偏弱。候選變化：分數已連續 2 日站在翻轉門檻另一側，再 1 日仍站住即翻爻。"],
];
