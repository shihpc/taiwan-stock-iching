...
2. **結果落在合理範圍**：`d_nonzero` 為 1.34～3.29，是設計起點值的 **0.55～0.67 倍**；對照原 p85 規則在 twse 給出的 0.29（起點值的 1/17，
...
原封不動套在有資訊量的樣本上。採用 d 是起點值的 0.55～0.67 倍（對照原 p85 規則在 twse 給出 0.29，是起點值的 1/17）。
