...
**c／權重／錨點／視窗皆為 P1 起點值；只有 `d` 已依訓練段校準**（2026-09-20，P3 第 3 項）——
採用值與逐類別規則在 `calibrated.py`（由 `scripts/apply_calibration.py` 從 `runs/calib/d_report_*.json` 產生），
本檔的每個 d 都寫成「查 `CALIBRATED_*`、查不到回退原設計起點值」，所以**沒被校準的鍵就是沒出現在那張表的鍵**。
`ParamSet.calibrated` 由 `CALIBRATION_META` 非空決定；`Param.calibrated`／`Rules.calibrated` 仍為 False
（前者是逐子指標欄位、本批未逐欄登錄，後者指的是 `Rules` 那組門檻常數——它們確實還沒校準）。
...
from .calibrated import CALIBRATED_D, CALIBRATED_DISTANCE_D, CALIBRATED_SLOPE_D, CALIBRATION_META
...
def cal_d(market: str, scope: str, indicator_id: str, horizon: str, default: float | None) -> float | None:
    """校準後的 d；`calibrated.py` 沒這個鍵就回退設計起點值 `default`（＝本檔原本寫死的那個數）。

    **不校準的鍵就是不在 `CALIBRATED_D` 裡的鍵**（`clip_policy=n/a` 者、以及訓練段 n=0 的
    `equity_qoq`／`vix_phist_rev`），它們一律走 `default`。距離／斜率走 `_cal_table`，不走這裡。"""
...
def _cal_table(table: str, market: str, start: dict[int, float]) -> dict[int, float]:
    ...
    **回傳新 dict**——B1 序言要求兩市場不得共用同一個設定物件。"""
    cal = CALIBRATED_DISTANCE_D.get(market, {}) if table == "distance_d" else CALIBRATED_SLOPE_D.get(table, {}).get(market, {})
    return {n: cal.get(n, v) for n, v in start.items()}
...
def _mk_market(market: str, dist: dict[int, float], mslope: dict[int, float],
    ...
    for h in HORIZONS:
        ...
        add(Param("new_high_low_ratio", "2", "C", h, sc, "S", 0.0, cal_d(market, sc, "new_high_low_ratio", h, 3.0), unit="%", window=n,
                  formula="(n 日新高家數 − n 日新低家數) ÷ N × 100"))
        add(Param("ad_line_dev", "2", "D", h, sc, "S", 0.0, cal_d(market, sc, "ad_line_dev", h, 1.0), unit="sd", window=n,
        ...
        add(Param("amount_ratio", "3", "A", h, sc, "S", 1.0, cal_d(market, sc, "amount_ratio", h, 0.30), unit="ratio", window=(num_n, den_n),
        ...
        # ---- B1.4 四爻 現貨資金
        n = MKT_WIN_N[h]
        add(Param("foreign_net_ratio", "4", "A", h, sc, "S", 0.0, cal_d(market, sc, "foreign_net_ratio", h, MKT_L4_D_FOREIGN[h]), unit="%", window=n,
                  formula="Σ外資淨買超金額 ÷ Σ市場成交金額 × 100", source_dataset="TWSE BFI82U / TPEx summary",
                  denominator_rule="Σ成交金額=0 → denominator_zero"))
        add(Param("trust_net_ratio", "4", "B", h, sc, "S", 0.0, cal_d(market, sc, "trust_net_ratio", h, MKT_L4_D_TRUST[h]), unit="%", window=n,
                  formula="Σ投信淨買超金額 ÷ Σ市場成交金額 × 100"))
        add(Param("foreign_buy_days", "4", "C", h, sc, "S", 0.0, cal_d(market, sc, "foreign_buy_days", h, 2.0), unit="day", window=10,
                  formula="近 10 日外資買超天數 − 5"))
        add(Param("margin_change", "4", "D", h, sc, "S", 0.0, cal_d(market, sc, "margin_change", h, 2.0), unit="%", window=n, direction=-1,
        ...
        add(Param("foreign_net_oi_change", "5", "A", h, sc, "S", 0.0, cal_d(market, sc, "foreign_net_oi_change", h, 5000.0), unit="口", window=n, sub_weight=.5,
                  formula="淨未平倉 n 日變化"))
        add(Param("basis", "5", "B", h, sc, "S", None, cal_d(market, sc, "basis", h, 0.30), unit="%", window=60,
        ...
        # ---- B1.6 上爻 外部（美股曆）
        add(Param("spx_return", "6", "A", h, sc, "S", 0.0, cal_d(market, sc, "spx_return", h, MKT_L6_D_SPX[h]), unit="%", window=n, sub_weight=.35,
        ...
        add(Param("sox_return", "6", "A", h, sc, "S", 0.0, cal_d(market, sc, "sox_return", h, MKT_L6_D_SOX[h]), unit="%", window=n, sub_weight=.35,
                  formula="費城半導體 n 個美股交易日報酬 × 100", source_dataset="USStockPrice(^SOX)"))
        add(Param("usdtwd_change", "6", "B", h, sc, "S", 0.0, cal_d(market, sc, "usdtwd_change", h, MKT_L6_D_FX[h]), unit="%", window=n, direction=-1,
...
def _mk_stock(market: str, dist: dict[int, float], sslope: dict[int, float],
    ...
    for h in HORIZONS:
        # ---- B2.1 初爻 營運基礎
        if h == "short":
            add(Param("revenue_yoy", "1", "A", h, sc, "S", 0.0, cal_d(market, sc, "revenue_yoy", h, 20.0), unit="pp", window=1,
            ...
            add(Param("revenue_yoy", "1", "A", h, sc, "S", 0.0, cal_d(market, sc, "revenue_yoy", h, 15.0), unit="pp", window=3,
                      formula="近 3 月合計 ÷ 去年同期 3 月合計 − 1 × 100", source_dataset="TaiwanStockMonthRevenue"))
        add(Param("revenue_accel", "1", "A", h, sc, "S", 0.0, cal_d(market, sc, "revenue_accel", h, 10.0), unit="pp", window=3,
        ...
        if h == "mid":
            ...
            add(Param("eps_yoy", "1", "B", h, sc, "S", 0.0, cal_d(market, sc, "eps_yoy", h, 20.0), unit="pp",
            ...
            add(Param("eps_diff_over_price", "1", "B", h, sc, "S", 0.0, cal_d(market, sc, "eps_diff_over_price", h, 2.0), unit="%",
                      formula="(本期EPS − 去年同期EPS) ÷ 期末股價 × 100（前期 EPS ≤ 0.1 或由負轉正）"))
            add(Param("gross_margin_qoq", "1", "B", h, sc, "S", 0.0, cal_d(market, sc, "gross_margin_qoq", h, 1.0), unit="pp", formula="本季毛利率 − 上季毛利率"))
            add(Param("pretax_income_yoy", "1", "B", h, sc, "S", 0.0, cal_d(market, sc, "pretax_income_yoy", h, 20.0), unit="pp",
            ...
            add(Param("equity_qoq", "1", "B", h, sc, "S", 0.0, cal_d(market, sc, "equity_qoq", h, 2.0), unit="%", formula="金融替代（industry_category ∈ {金融保險, 金融業}）：淨值 QoQ × 100",
            ...
            add(Param("revenue_yoy_vs_industry", "1", "C", h, sc, "S", 0.0, cal_d(market, sc, "revenue_yoy_vs_industry", h, 10.0), unit="pp", window=3,
        ...
        add(Param("excess_long", "3", "A", h, sc, "S", 0.0, cal_d(market, sc, "excess_long", h, d_l), unit="pp", window=wl, formula="長視窗超額報酬（對所屬市場指數）"))
        add(Param("excess_short", "3", "A", h, sc, "S", 0.0, cal_d(market, sc, "excess_short", h, d_s), unit="pp", window=ws, formula="短視窗超額報酬"))
        add(Param("excess_vs_industry", "3", "B", h, sc, "S", 0.0, cal_d(market, sc, "excess_vs_industry", h, d_ind), unit="pp", window=wl,
                  formula="長視窗超額（對產業中位）", missing_rule="同產業有效樣本 < 5 → 族缺"))
        add(Param("excess_accel", "3", "C", h, sc, "S", 0.0, cal_d(market, sc, "excess_accel", h, d_acc), unit="pp", window=ws,
                  formula="最近 ws 日超額 − 其前 ws 日超額"))
        FW[(sc, h, "3")] = {"A": .50, "B": .25, "C": .25}
        # ---- B2.4 四爻 量價確認
        dd_n, cp_n, base_n, confirm_k = STK_L4_WIN[h]
        if h == "mid":
            add(Param("updown_volume_ratio", "4", "A", h, sc, "S", 0.0, cal_d(market, sc, "updown_volume_ratio", h, 0.3), unit="ln", window=20, sub_weight=.5,
                      formula="ln(近 20 日上漲日均量 ÷ 下跌日均量)", denominator_rule="無上漲日或無下跌日 → denominator_zero"))
            add(Param("obv_slope", "4", "A", h, sc, "S", 0.0, cal_d(market, sc, "obv_slope", h, 0.5), unit="ratio", window=20, sub_weight=.5,
        ...
        for fam, who in (("A", "foreign"), ("B", "trust")):
            add(Param(f"{who}_strength_long", "5", fam, h, sc, "S", 0.0, cal_d(market, sc, f"{who}_strength_long", h, dl5), unit="%", window=wl5, sub_weight=.6,
            ...
            add(Param(f"{who}_strength_short", "5", fam, h, sc, "S", 0.0, cal_d(market, sc, f"{who}_strength_short", h, ds5), unit="%", window=ws5, sub_weight=.4,
                      formula="同上（短視窗）"))
        add(Param("foreign_persistence", "5", "C", h, sc, "S", 0.0, cal_d(market, sc, "foreign_persistence", h, dp5), unit="day", window=wn5,
        ...
        add(Param("margin_scenario", "5", "D", h, sc, "scenario", native_range=S_RANGE, clip_policy="clip_3d",
                  c=0.0, d=cal_d(market, sc, "margin_scenario", h, 5.0), unit="%", window=wn5,
                  formula="S1 §A2.1 有序情境；序 3／4 為 S(−r; 0, 5%) 及其半幅仿射，原生值域即 S 值域、N 恆等",
                  missing_rule="無信用交易資格 → 族缺"))
        add(Param("short_sale_change", "5", "E", h, sc, "S", 0.0, cal_d(market, sc, "short_sale_change", h, 0.3), unit="pp", window=wn5, direction=-1,
        ...
        add(Param("industry_relative_return", "6", "B", h, sc, "S", 0.0, cal_d(market, sc, "industry_relative_return", h, STK_L6_D[h]), unit="pp", window=STK_L6_WIN[h],
