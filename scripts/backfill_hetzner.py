...
def run_dataset(spec: C.DatasetSpec, strategy: str, stores: dict[str, Store], fm: FinMind | None,
    ...
    partial_keys: set[str] = set()               # --data-end 下的「延伸塊」：有舊鍵者 ∪ 迄日不在 chunk 自然期末者（empty_ok_partial 用）
    if data_end:
        ...
        partial_keys = set(replace_of) | {k for k, o in shifts if o is None and P.key_is_partial_block(spec, k)}
        ...
        try:
            if spec.source == "finmind":
                ...
                if not rows and strategy not in spec.empty_ok_for and spec.empty_ok_partial and key in partial_keys:
                    # --data-end 延伸塊（期別未結束）回空：視為合法 empty、寫 coverage=empty（config.DatasetSpec.empty_ok_partial）。
                    ...
                    log.info("[%s] %s 延伸塊回空（期別未結束）→ coverage=empty（empty_ok_partial；舊鍵 %s 保留，下次延伸再一併取代）",
    ...
    for r in results:
        ...
        if data_end and "refetch" in r:
            ...
            if r.get("empty_partial"):
                line += f" 延伸塊空(合法)={r['empty_partial']:>3}"
