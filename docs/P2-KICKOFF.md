...
| 9 | 大盤開盤價來源 | **甲：以證據定**——回補腳本附證交所官方日 OHLC 比對，與官方一致者為準；皆不一致再回問 **證據已到（2026-09-12 Hetzner 實測，本裁定據此結案）**：`taiex-open-check` 對證交所 `MI_5MINS_HIST` 官方開盤指數逐日比對 2022-01-01~2026-08-31 共 1,129 個交易日（一致＝絕對差 ≤ 0.005）：`TaiwanStockPrice`/`TAIEX` 的 `open` **1,129/1,129＝100.00%、最大絕對差 0.00**（逐位相同，不只是落在容差內）；`TaiwanStockKBar` 09:00 bar 的 `close` **0/1,129＝0.00%、最大絕對差 2,071.42**。**採宣告源的 `open`**，已寫入 `docs/pre-registration.md` §1.2 表格與 §1.2.3「定案」小節。殘留：比對自 2022-01-01 起，**2020–2021 兩年未比對**（`--month-from 202001` 約 8 分鐘可補），凍結前補上或明載涵蓋範圍。 |
