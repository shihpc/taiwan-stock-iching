...
**純函式＋只含 deque 的有狀態類別，不 import sqlite3**（兩層 parity 的硬約束，`spec/P1-B3-replay.md` §B3.2）。
...
2. **後復權價**（`adjust.py`）。漲跌、MA、新高低**全部同一個口徑**——這是刻意的：
   ...
   **SPEC-NOTE**：官方「上漲家數」是原始價口徑，本模組刻意不同；差異只落在除權息日，
   ...
   （開關＝`ADVANCE_ON_ADJUSTED`；**目前這個常數只是宣告口徑、還沒有第二條分支在用它**）。
3. **站上 MA_n**＝`close_adj > MA_n`（**嚴格大於**）。等於 MA 不算站上。浮點相等機率極低，
   但漲停鎖死／長期不動的個股會出現，固定為嚴格以免兩層 parity 靠平手行為分歧。
...
5. **家數比的分母一律是 `n_stocks`（N_t）**，照 `P1-B1-market.md:157`（「分子分母必須同一份名單」）。
   歷史不足 n 天的新股算在分母、不可能在分子——這是規格的讀法，偏誤大小由 `ma_eligible`
   等診斷欄量測（穩態下應 <1%）。**所有原始計數都輸出**，日後改口徑不必重掃。
...
MA_WINDOWS: tuple[int, ...] = (5, 10, 20, 60)          # MarketInputs.above_ma_ratio 的鍵（MKT_L2_WIN 用到的 MA_短／MA_長）
HL_WINDOWS: tuple[int, ...] = (10, 20, 60)             # MarketInputs.new_high_low_ratio 的鍵（＝MKT_L2_WIN 的 n）
RET_WINDOWS: tuple[int, ...] = (10, 20, 60)            # STK_L3_WIN 的長視窗 wl：short 10／swing 20／mid 60
HORIZON_BY_RET_WINDOW = {10: "short", 20: "swing", 60: "mid"}
ADVANCE_ON_ADJUSTED = True                             # 見模組 docstring 第 2 點（口徑開關）
...
class IndustryAgg:
    """產業聚合（`score/stock.py:ind_excess_vs_industry` 的 `industry_median_ret`／`industry_n`）。
...
class ScanDay:
    ...
    """該日缺指數收盤的市場。**指數 deque 不推進**，於是其後的 n 日指數報酬會跨越多於 n 個交易日
    ——這種扭曲無聲，所以在此據實列出，由呼叫端記進 report／failures，不得吞掉。"""
...
class DailyScanner:
    ...
    ＋每市場一個 AD 累積值。約 2,000 檔 × 61 × 8 bytes ≈ 1 MB。
    """

    def __init__(self, ma_windows: Iterable[int] = MA_WINDOWS, hl_windows: Iterable[int] = HL_WINDOWS,
                 ret_windows: Iterable[int] = RET_WINDOWS, advance_on_adjusted: bool = ADVANCE_ON_ADJUSTED) -> None:
        ...
        self.advance_on_adjusted = bool(advance_on_adjusted)
    ...
    def push_day(self, tpe_date: str, rows: Iterable[StockDay],
        ...
        rows = sorted(rows, key=lambda r: str(r.stock_id))          # 決定性走訪序（見 docstring 第 11 點）
        ...
        for mk in sorted(set(list(breadth) + list(index_close))):
            ...
            for n in self.ret_windows:
                ...
                for sid, sret in sorted(rets.get((mk, n), {}).items()):
                    ex = sret - mret
                    rank_rets.setdefault((mk, n), {})
                    self._excess_put(rank_rets, mk, n, sid, ex)

        # 產業中位數（母體＝廣度母體）／P_cs（母體＝排名池）
        in_pool = {str(r.stock_id) for r in rows if r.in_rank_pool}
        industry_of = {str(r.stock_id): (r.industry or "") for r in rows}
        for (mk, n), per in sorted(rank_rets.items()):
            for sid, ex in sorted(per.items()):
                ind = industry_of.get(sid) or ""
                if ind:
                    ind_rets.setdefault((mk, n, ind), []).append(ex)
