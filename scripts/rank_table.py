...
def _type_clause(n_pos_neg: int, n_neg_pos: int) -> str:
    """異號卦的型態敘述——**由計數判定，不得硬編全稱句**。

    首版寫死「**全部是**「均值正、中位數負」這一型（{n_pos_neg} 個；相反的 {n_neg_pos} 個）」。
    造一個 `mean<0<median` 的卦實跑，輸出是「全部是 X（**0** 個；相反的 **1** 個）」——
    **字面自我矛盾**，28 支測試全綠。這與 F1（硬編方向被自己的表推翻）是同一個失效模式，
    而且就出現在修 F1 的那段程式裡；`:277` 上方那行「方向必須逐格由資料判定，不得硬編」
    的註解，當時漏掉了這一句（2026-09-23 複驗實測）。
    """
    tail = ("——報酬右偏，少數大漲把均值拉正而中位數仍為負。"
            if n_pos_neg >= n_neg_pos else "——左偏，少數大跌把均值拉負而中位數仍為正。")
    # 單一型時**仍列出另一型的 0**：那是「有查過」的證據，拿掉等於讓讀者不知道有沒有查
    if n_pos_neg and not n_neg_pos:
        return (f"本份資料的異號卦**全部是「均值正、中位數負」**這一型"
                f"（{n_pos_neg} 個；相反的「均值負、中位數正」{n_neg_pos} 個）{tail}")
    if n_neg_pos and not n_pos_neg:
        return (f"本份資料的異號卦**全部是「均值負、中位數正」**這一型"
                f"（{n_neg_pos} 個；相反的「均值正、中位數負」{n_pos_neg} 個）"
                "——左偏，少數大跌把均值拉負而中位數仍為正。")
    if not n_pos_neg and not n_neg_pos:
        return "本份資料**沒有異號卦**，高／低組不受此規則影響。"
    return (f"本份資料的異號卦**兩型都有**：「均值正、中位數負」{n_pos_neg} 個、"
            f"「均值負、中位數正」{n_neg_pos} 個{tail}")
...
def as_markdown(rep: dict[str, Any]) -> str:
    ...
    L += ["### ⚠ 高組與低組**不對稱**（讀表前必看）", "",
          ...
          "**這是規則的算術後果，不是訊號強弱。**", "",
          f"機制：{_type_clause(n_pos_neg, n_neg_pos)}它們照常排名，但落在高／低組區間時"
          "會被拉回中組，**名額不遞補**，所以那一端就少幾卦。", "",
          "**落在哪一端逐格不同**，取決於同號卦的報酬水準（下表 `異號名次中位數` 與 "
          "`同號名次中位數` 的相對位置）："]
    tie = [k for k in sorted(cells) if k not in front and k not in back]
    if front:
    ...
    if back:
        L.append(f"- **{'、'.join(f'`{k}`' for k in back)}**：同號卦均值普遍較高，"
                 "異號卦相對靠後 → 被掏空的是**低組**")
    if tie:
        # 平手（兩邊名次中位數相同）或該格沒有異號卦／沒有同號卦。**不能靜默**——
        # 上一行的冒號已經承諾要逐格說明，少一格就是懸空（2026-09-23 複驗實測構造出來的）。
        L.append(f"- **{'、'.join(f'`{k}`' for k in tie)}**：兩者名次中位數相同或缺一方，"
                 "分不出方向（不歸前段也不歸後段）")
